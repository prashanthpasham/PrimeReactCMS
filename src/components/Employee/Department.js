...
export default class Department extends Component
{
    ...
    saveDept(){
      ...
      if(this.state.deptCode==undefined || this.state.deptCode.trim().length==0){
        ...
        this.carService.postServer("login/add-department",localStorage.getItem("token"),department).then((res)=>{
        if(res!=undefined){
    ...
    delete(){
        this.carService.getAllClients("login/delete-department/"+this.state.deptId,localStorage.getItem("token")).
     then(res=>{
