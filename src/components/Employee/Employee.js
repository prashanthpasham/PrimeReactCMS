        ...
        if(Object.keys(this.state.seldesignation).length>0)
          {
           ...
           //   alert(employee.employeeId);
              ...
              this.carService.postServer("login/add-employee",localStorage.getItem("token"),employee) 
             .then(res=>{
...
onDepartment(e){
  ...
  back(){
      ...
      this.props.history.push("/app/employee-list");
  }
render(){
    ...
    return (<div className="p-grid">
        ...
        <div>
        <Button className="p-button-danger"  label="Back" style={{padding:'5px',width:'100px',float:'right',marginRight:'15%',marginTop:'-60px'}} onClick={()=>this.back()}/>
        <Button label="Save" style={{padding:'5px',marginRight:'5%',marginTop:'-60px',width:'100px',float:'right'}} className="p-button-primary" onClick={()=>this.addEmployee()}></Button>
        </div> 
